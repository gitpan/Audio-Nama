...
sub has_read_only_param {
	...
	logit(__LINE__,'Audio::Nama::Effects','logcluck',"undefined or unregistered effect id: $id"), 
...
sub catch_null_id {
	my $id = shift;
	logit(__LINE__,'Audio::Nama::Effects','logconfess',"null effect id") unless $id;
}
...
sub set_chain_value {
	...
	#logit(__LINE__,'Audio::Nama::Effects','debug',(yaml_out($p));
...
sub add_effect {
	my $p = shift;
	logsub("&add_effect");
	#logit(__LINE__,'Audio::Nama::Effects','debug',sub{ "add effect arguments - 0:\n".yaml_out($p)});
	
	set_chain_value($p);

	logit(__LINE__,'Audio::Nama::Effects','debug',sub{ "add effect arguments - 1:\n".yaml_out($p)});
...
sub _insert_effect {  # call only from add_effect
	...
	my $track = $ti{$n};
	#logit(__LINE__,'Audio::Nama::Effects','debug', $track->name, $/;
	#logit(__LINE__,'Audio::Nama::Effects','debug', "@{$track->ops}")
	...
	logit(__LINE__,'Audio::Nama::Effects','debug',"ops to remove and re-apply: @ops");
	...
	logit(__LINE__,'Audio::Nama::Effects','debug',sub{"@{$track->ops}"});
...
sub modify_effect {
	...
	$this_op = $op_id;
	logit(__LINE__,'Audio::Nama::Effects','debug', "id $op_id p: $parameter, sign: $sign value: $value");
...
sub remove_effect { 
	...
	my $id = shift;
	if( ! fx($id) ){
		logit(__LINE__,'Audio::Nama::Effects','logcarp',"$id: does not exist, skipping...\n");
		return;
	}
	my $n 		= chain($id);
	my $parent 	= parent($id);
	my $owns	= owns($id);
	logit(__LINE__,'Audio::Nama::Effects','debug', "id: $id, parent: $parent");

	my $object = $parent ? q(controller) : q(chain operator); 
	logit(__LINE__,'Audio::Nama::Effects','debug', qq(ready to remove $object "$id" from track "$n"));

	$ui->remove_effect_gui($id);

	# recursively remove children
	logit(__LINE__,'Audio::Nama::Effects','debug',"children found: ". join ",",@$owns) if defined $owns;
...
;
	...
	# remove controller
		...
		logit(__LINE__,'Audio::Nama::Effects','debug',"parent $parent owns: ". join ",", @$parent_owns);

		@$parent_owns = (grep {$_ ne $id} @$parent_owns);
		logit(__LINE__,'Audio::Nama::Effects','debug',"parent $parent new owns list: ". join ",", @$parent_owns);
...
sub nama_effect_index { # returns nama chain operator index
	...
	my $n = chain($id);
	my $arr = $ti{$n}->ops;
	logit(__LINE__,'Audio::Nama::Effects','debug', "id: $id n: $n");
	logit(__LINE__,'Audio::Nama::Effects','debug', "@{$ti{$n}->ops}" );
...
sub ecasound_effect_index { 
	...
	logit(__LINE__,'Audio::Nama::Effects','debug', "id: $id, n: $n, ops: @{ $ti{$n}->ops }" );
	for my $op (@{ $ti{$n}->ops }) { 
...
sub apply_ops {  # in addition to operators in .ecs file
	...
	logit(__LINE__,'Audio::Nama::Effects','debug', "chain: $n, offset: $fx->{offset}->{$n}");
...
sub apply_op {
	...
	logit(__LINE__,'Audio::Nama::Effects','debug', "id: $id");
	my $code = type($id);
	my $dad = parent($id);
	my $chain = chain($id);
	logit(__LINE__,'Audio::Nama::Effects','debug', "chain: ".chain($id)." type: $code");
	...
	$code = '-' . $code . ($code =~ /:/ ? q(,) : q(:) );
	my @vals = @{ params($id) };
	logit(__LINE__,'Audio::Nama::Effects','debug', "values: @vals");
	...
	logit(__LINE__,'Audio::Nama::Effects','debug', "command: $add_cmd");
	...
	my @owns = @{ owns($id) }; 
	logit(__LINE__,'Audio::Nama::Effects','debug',"children found: ". join ",", @{owns($id)});
...
sub remove_op {
	...
	if ( ! is_controller($id) ){ # chain operator
		logit(__LINE__,'Audio::Nama::Effects','debug', "no parent, assuming chain operator");
	
		$index = ecasound_effect_index( $id );
		logit(__LINE__,'Audio::Nama::Effects','debug', "ops list for chain $n: @{$ti{$n}->ops}");
		logit(__LINE__,'Audio::Nama::Effects','debug', "operator id to remove: $id");
		logit(__LINE__,'Audio::Nama::Effects','debug', "ready to remove from chain $n, operator id $id, index $index");
		logit(__LINE__,'Audio::Nama::Effects','debug',sub{eval_iam("cs")});
		eval_iam("cop-select ". ecasound_effect_index($id) );
		logit(__LINE__,'Audio::Nama::Effects','debug',sub{"selected operator: ". eval_iam("cop-selected")});
		eval_iam("cop-remove");
		logit(__LINE__,'Audio::Nama::Effects','debug',sub{eval_iam("cs")});

	} else { # controller

		logit(__LINE__,'Audio::Nama::Effects','debug', "has parent, assuming controller");

		my $ctrl_index = ctrl_index($id);
		logit(__LINE__,'Audio::Nama::Effects','debug', eval_iam("cs"));
		eval_iam("cop-select ".  ecasound_effect_index(root_parent($id)));
		logit(__LINE__,'Audio::Nama::Effects','debug', "selected operator: ". eval_iam("cop-selected"));
...
sub cop_add {
	logsub("&cop_add");
	my $p = shift;
	logit(__LINE__,'Audio::Nama::Effects','debug',sub{yaml_out($p)});
	...
	#
	logit(__LINE__,'Audio::Nama::Effects','debug',"$id: returning existing id") if $id and fx($id);
	return $id if $id and fx($id);
	
	$id = $p->{cop_id} = $fx->{id_counter}  ;
	logit(__LINE__,'Audio::Nama::Effects','debug',"$id: cop id issued");

	my $i = effect_index($type);

	logit(__LINE__,'Audio::Nama::Effects','debug',"Issuing a cop_id for track $n: $id");
	...
	if (! $parent_id and ! $p->{values}){
		my @vals;
		logit(__LINE__,'Audio::Nama::Effects','debug', "no settings found, loading defaults if present");
		...
		logit(__LINE__,'Audio::Nama::Effects','debug', "copid: $id defaults: @vals");
		$p->{values} = \@vals;
	}
	...
	if ($parent_id) {
		logit(__LINE__,'Audio::Nama::Effects','debug', "parent found: $parent_id");

		# store relationship

		push @{ owns($parent_id) }, $id;
		logit(__LINE__,'Audio::Nama::Effects','debug',"parent owns @{owns($parent_id)}");

		logit(__LINE__,'Audio::Nama::Effects','debug',sub{join " ", "my attributes:", yaml_out(fx($id))});
		parent($id) = $parent_id;
		logit(__LINE__,'Audio::Nama::Effects','debug',sub{join " ", "my attributes again:", yaml_out(fx($id))});
		#logit(__LINE__,'Audio::Nama::Effects','debug', "parameter: $parameter");
...
sub effect_update {
	...
	#logit(__LINE__,'Audio::Nama::Effects','debug', "engine is $es");
	...
	logit(__LINE__,'Audio::Nama::Effects','debug', "chain $chain id $id param $param value $val");
	...
	if( is_controller($id)){
		...
		logit(__LINE__,'Audio::Nama::Effects','debug', "controller $id: track: $chain, index: $i param: $param, value: $val");
		...
		logit(__LINE__,'Audio::Nama::Effects','debug', "operator $id: track $chain, index: $i, offset: ".
...
sub find_op_offsets {
	...
	logit(__LINE__,'Audio::Nama::Effects','debug', join "\n\n",@op_offsets);
	for my $output (@op_offsets){
		...
		next if $chain_id =~ m/\D/; # skip id's containing non-digits
									# i.e. M1
		my $quotes = $output =~ tr/"//;
		logit(__LINE__,'Audio::Nama::Effects','debug', "offset: $quotes in $output");
