...
sub catch_null_id {
	return 0;
	my $id = shift;
	logpkg(__FILE__,__LINE__,'logconfess',"null effect id")  unless $id;
	logpkg(__FILE__,__LINE__,'debug',"$id: effect id does not exist") 
...
sub has_read_only_param {
	...
	logpkg(__FILE__,__LINE__,'logcluck',"undefined or unregistered effect id: $op_id"), 
...
sub set_chain_value {
	...
	#logpkg(__FILE__,__LINE__,'debug',(yaml_out($p));
...
sub _add_effect { 
	...
	if( valid_engine_setup() )
	{
		...
		{ 
			$ti{$n}->mute;
			Audio::Nama::stop_do_start( sub{ apply_op($id) }, 0.05);
			$ti{$n}->unmute;
		}
		else { apply_op($id) }
	}
...
sub _insert_effect {  # call only from add_effect
	...
	logpkg(__FILE__,__LINE__,'debug',"ops to remove and re-apply: @ops");
...
sub remove_effect { 
	...
	my $id = shift;
	if( ! $id or ! fx($id) ){
		logpkg(__FILE__,__LINE__,'logcarp',"$id: does not exist, skipping...\n");
		return;
	}
	my $n 		= chain($id);
	$n or die Audio::Nama::json_out(fx($id));
	my $parent 	= parent($id);
	my $owns	= owns($id);
...
sub apply_op {
	...
	my $dad = parent($id);
	my $chain = chain($id);
	logpkg(__FILE__,__LINE__,'debug', "chain: ".chain($id)." type: $code");
	...
	my @owns = @{ owns($id) }; 
	logpkg(__FILE__,__LINE__,'debug',"children found: ". join ",", @{owns($id)});
...
sub remove_op {
	...
	if ( ! is_controller($id) ){ # chain operator
		...
		logpkg(__FILE__,__LINE__,'debug',sub{eval_iam("cs")});
		eval_iam("cop-select ". ecasound_effect_index($id) );
		logpkg(__FILE__,__LINE__,'debug',sub{"selected operator: ". eval_iam("cop-selected")});
		eval_iam("cop-remove");
		logpkg(__FILE__,__LINE__,'debug',sub{eval_iam("cs")});
		...
		logpkg(__FILE__,__LINE__,'debug', eval_iam("cs"));
		eval_iam("cop-select ".  ecasound_effect_index(root_parent($id)));
		logpkg(__FILE__,__LINE__,'debug', "selected operator: ". eval_iam("cop-selected"));
...
sub effect_init {
	logsub("&effect_init");
	my $p = shift;
	logpkg(__FILE__,__LINE__,'debug',sub{yaml_out($p)});
	...
	#
	logpkg(__FILE__,__LINE__,'debug',"$id: returning existing id") if $id and fx($id);
	...
	if ($parent_id) {
		...
		logpkg(__FILE__,__LINE__,'debug',"parent owns @{owns($parent_id)}");

		logpkg(__FILE__,__LINE__,'debug',sub{join " ", "my attributes:", yaml_out(fx($id))});
		parent($id) = $parent_id;
		logpkg(__FILE__,__LINE__,'debug',sub{join " ", "my attributes again:", yaml_out(fx($id))});
...
sub effect_update {
	...
	if( is_controller($id)){
		...
		logpkg(__FILE__,__LINE__,'debug', "operator $id: track $chain, index: $i, offset: ".
