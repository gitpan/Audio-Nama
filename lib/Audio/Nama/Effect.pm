...
sub new {
	...
	if ($self = fxn($id)){
		logpkg(__FILE__,__LINE__,'debug',"$id: returning existing object"); 
		return $self
	}
	...
	logpkg(__FILE__,__LINE__,'debug',"$id: effect id $how_allocated");

	my $i = effect_index($args{type});
	defined $i or confess "$args{type}: effect index not found.";

	logpkg(__FILE__,__LINE__,'debug',"$id: Issuing effect id for track $args{chain}");
	...
	if ($parent_id) {
		...
		logpkg(__FILE__,__LINE__,'debug',sub{join " ", "my attributes again:", json_out($self->as_hash)});
