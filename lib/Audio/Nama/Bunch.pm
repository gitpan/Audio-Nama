...
sub bunch_tracks {
	...
	if ( my $bus = $bn{$bunchy}){
		...
		logit(__LINE__,'Audio::Nama::Bunch','debug', "special bunch: bus");
		@tracks = grep{ ! $bn{$_} } $bn{$this_bus}->tracks;
	} elsif ($bunchy =~ /\s/  # multiple identifiers
		or $tn{$bunchy} 
		or $bunchy !~ /\D/ and $ti{$bunchy}){ 
			logit(__LINE__,'Audio::Nama::Bunch','debug', "multiple tracks found");
		...
		logit(__LINE__,'Audio::Nama::Bunch','debug', "special bunch: $bunchy, method: $method");
		$bunchy = uc $bunchy;
		@tracks = grep{$tn{$_}->$method eq $bunchy} 
				$bn{$this_bus}->tracks
	} elsif ( $project->{bunch}->{$bunchy} and @tracks = @{$project->{bunch}->{$bunchy}}  ) {
		logit(__LINE__,'Audio::Nama::Bunch','debug', "bunch tracks: @tracks");
