...
sub reconfigure_engine {
	...
	if( $setup->{changed} ){ 
		...
		if ( $current eq $old){
				logit(__LINE__,'Audio::Nama::Engine_setup','debug',"no change in setup");
				return;
		}
	}
	logit(__LINE__,'Audio::Nama::Engine_setup','debug',"setup change");
...
# 		say "restore_position: $restore_position";
	...
	if ( generate_setup() ){
		...
		logit(__LINE__,'Audio::Nama::Engine_setup','debug',"I generated a new setup");
		...
		git_snapshot() unless Audio::Nama::ChainSetup::really_recording(); 
...
sub load_ecs {
	...
	logit(__LINE__,'Audio::Nama::Engine_setup','debug',sub{map{eval_iam($_)} qw(cs es fs st ctrl-status)});
	1;
}
sub teardown_engine {
	eval_iam("cs-disconnect") if eval_iam("cs-connected");
